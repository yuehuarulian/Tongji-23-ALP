...
/* 可以在此定义其它需要的函数 */
int year(int& tj_year, int& input_time)
{
	tj_year = 1970;
	int r;
	for (int i = 0; ; i++) {
		if ((tj_year % 400 == 0 || (tj_year % 100 != 0 && tj_year % 4 == 0)) && input_time >= 366) {
			input_time -= 366;
			tj_year++;;
		}
	...
	r = tj_year % 400 == 0 || (tj_year % 100 != 0 && tj_year % 4 == 0);
	return r;
}
void month(int& tj_month, int& input_time, int r)
{
...
{
	if ((prompt == NULL) || (strlen(prompt) == 0)) //思考一下，||的左右两个条件能否互换  不能
		cout << endl << "按回车键继续";
	else
...
struct tj_time* tj_time_convert(int input_time)
{
	...
	input_time /= 24;//单位天
	int r = year(result.tj_year, input_time);
	month(result.tj_month, input_time, r);
